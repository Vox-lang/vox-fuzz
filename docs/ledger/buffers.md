...
| BUF-19 | 3712 | Byte positions are 1-indexed. | on a buffer with known content, assert `byte 1 of buf` equals the first byte's known value | yes | `gen leaf buffer byte index` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 41/200 |
...
| BUF-23 | 3722 | Writing a byte with a hex literal (`0x48`). | — | yes | `gen leaf buffer byte literals` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 38/200 |
| BUF-24 | 3723 | Writing a byte with a character literal (`'A'`). | — | yes | `gen leaf buffer byte literals` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 38/200 |
...
| BUF-26 | 3742 | Out-of-bounds access sets an error flag **and returns 0**. | capture the OOB read into a variable, assert it equals 0 | yes | `gen leaf buffer oob value` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 20/200 |
...
| BUF-31 | 3777 | `append source to destination` adds source bytes to the end of destination. | assert `dest == old_dest + source` and `dest's size == old_size + source_len` | yes | `gen leaf buffer append copy sources` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 13/200 |
| BUF-32 | 3778 | `copy source to destination` replaces destination contents with source bytes. | assert `dest == source` after copy | yes | `gen leaf buffer append copy sources` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 13/200 |
...
| BUF-38 | 3784 | Source buffer/text is never modified by append or copy. | print source before and after, assert unchanged | yes | `gen leaf buffer append copy sources` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 26/200 |
...
| BUF-42 | 3647 | All three spellings accept an optional article `the` (`Free the X.`). | emit `the` before the operand for each spelling | yes | `gen leaf buffer free spellings` | exercised | construct emitted by gen leaf buffer free spellings (spelling/position drawn); the documented result is asserted through a sibling row's check, not its own |
| BUF-43 | 3655–3656 | After `Free`, the buffer is empty: `size`/`length` read 0. | assert `X's size is 0` and `X's length is 0` right after `Free` | yes | `gen leaf buffer freed state` | exercised | leaf assertion, per-surface gate test proves it fires (draw missed in the 200-program 2026-09-05 campaign) |
| BUF-44 | 3657–3659 | A freed buffer behaves exactly like a fixed buffer of capacity 0 — `capacity` reads 0, `empty` is true. Hand-verified extra consequence the sentence implies but doesn't spell out: `full` also reads true (0 == 0). | assert capacity 0, empty true, full true after `Free` | yes | `gen leaf buffer freed state` | exercised | leaf assertion, per-surface gate test proves it fires (draw missed in the 200-program 2026-09-05 campaign) |
| BUF-45 | 3656 | After `Free`, `X as text` reads back `""` (the empty string). | assert `X as text is ""` | yes | `gen leaf buffer freed state` | exercised | leaf assertion, per-surface gate test proves it fires (draw missed in the 200-program 2026-09-05 campaign) |
