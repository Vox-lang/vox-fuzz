...
| FUN-32 | 850 | In expressions, a call is the function name followed by `of`, `to`, `with`, or `on` and its arguments. | emit each of the four connectors | yes — every connector must produce the same result | **`of` only** — `Print f2 of c{n}` (`src/gen_core.vox:374`), `Print f3 of ... and ...` (`:387`), `f4 of "..."` (`src/gen_text.vox:408`), `'grid sink N' of ...` (`src/gen_flow.vox:105`), `f3 of each ... from ...` (`src/gen_flow.vox:55,218`). **`to`, `with` and `on` are never emitted.** | exercised (`of`); todo (the other three) — real gap, all four hand-verified identical | |
| FUN-33 | 853–855 | The three example call fragments parse and evaluate: `'add numbers' of 3 and 5`, `'check divisibility' of the number and 6`, `calculate with x and y`. | reproduce all three | yes | partially — the first shape is `f3 of c1 and c2`; the `the <name>` argument form and the `with` connector are not emitted | todo | |
| FUN-34 | 859 | (Call site) the function name is a bare single word or a single-quoted multi-word name. | — | — | — | folded into FUN-05 (the same rule, restated for the call site; both forms are exercised) | |
| FUN-35 | 860 | For calls with arguments, use `of`, `to`, `with`, or `on`. | — | — | — | folded into FUN-32 | |
| FUN-36 | 861 | Multiple arguments are separated by `and`. | emit a call with two or more arguments | yes | `Print f3 of c{a} and c{b}` (`src/gen_core.vox:387`), `'grid sink N' of ...` with 3–17 clauses (`src/gen_flow.vox:105`) | exercised | |
| FUN-37 | 863–868 | Calls with no arguments can be written directly, as a bare sentence (`'show version'.`, `ping.`). | emit a bare no-argument call statement | yes — assert what the callee printed or changed | `'read flags {n}'` as its own line (`src/gen_misc.vox:415`); the expression-position variant of the same form, `Print f1` (`src/gen_core.vox:366`), is also emitted | exercised | |
...
LANGUAGE.md:757: "Variables declared at top level are global and can be
...
it must not keep doing is what LANGUAGE.md:702-705 says the 0.3.0
...
without ever reaching a campaign. LANGUAGE.md:757–763 now states the fixed
...
LANGUAGE.md:782: a typed parameter "supports the same properties and
...
runtime in. LANGUAGE.md:801–805 now documents map parameters explicitly
...
LANGUAGE.md:805: "file parameters support file properties." Two of the
...
LANGUAGE.md:783-785 lists `file` among the eleven legal return types;
...
that reading LANGUAGE.md:816-818 is imprecise rather than false ("tracked
...
LANGUAGE.md:764 promises only that this is a **compile-time error**, and
...
- a blank line after every function definition — LANGUAGE.md:720–725 (the
  manual's own two-definition example), and the compiler's own warning
  quoted in `FUN-43.vox`; FUN-43
- every function definition begins with `To ` — LANGUAGE.md:714, FUN-01
- every parameter is written `a <type> called <name>` — LANGUAGE.md:737,
  FUN-07
- parameters in a definition are joined with `and`, and arguments at a
  call site are separated with `and` — LANGUAGE.md:738 and :770, FUN-08,
  FUN-36
- a declared return type is always written `Return a <type>,` —
  LANGUAGE.md:739, FUN-09
- a function or parameter name containing a space is always
  single-quoted — LANGUAGE.md:735 and :699, FUN-05, FUN-07. Note the
...
- every emitted call uses the connector `of` — LANGUAGE.md:860 accepts
...
the disease LANGUAGE.md:694-705 says 0.3.0 was written to cure.
...
1. **Read properties off a parameter, not just a top-level variable.**
   ...
   LANGUAGE.md:782-783 that reads like boilerplate. That sentence is the
