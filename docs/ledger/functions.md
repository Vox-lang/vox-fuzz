...
LANGUAGE.md:773: "Variables declared at top level are global and can be
...
it must not keep doing is what LANGUAGE.md:718-721 says the 0.3.0
...
without ever reaching a campaign. LANGUAGE.md:773–779 now states the fixed
...
LANGUAGE.md:798: a typed parameter "supports the same properties and
...
parameter-type list (LANGUAGE.md:796–798, "plan 296") ahead of the
...
runtime in. LANGUAGE.md:817–821 now documents map parameters explicitly
...
LANGUAGE.md:821: "file parameters support file properties." Two of the
...
LANGUAGE.md:799-801 lists `file` among the eleven legal return types;
...
that reading LANGUAGE.md:832-834 is imprecise rather than false ("tracked
...
LANGUAGE.md:780 promises only that this is a **compile-time error**, and
...
- a blank line after every function definition — LANGUAGE.md:736–741 (the
  manual's own two-definition example), and the compiler's own warning
  quoted in `FUN-43.vox`; FUN-43
- every function definition begins with `To ` — LANGUAGE.md:730, FUN-01
- every parameter is written `a <type> called <name>` — LANGUAGE.md:753,
  FUN-07
- parameters in a definition are joined with `and`, and arguments at a
  call site are separated with `and` — LANGUAGE.md:754 and :770, FUN-08,
  FUN-36
- a declared return type is always written `Return a <type>,` —
  LANGUAGE.md:755, FUN-09
- a function or parameter name containing a space is always
  single-quoted — LANGUAGE.md:751 and :699, FUN-05, FUN-07. Note the
...
- every emitted call uses the connector `of` — LANGUAGE.md:876 accepts
...
the disease LANGUAGE.md:710-721 says 0.3.0 was written to cure.
...
1. **Read properties off a parameter, not just a top-level variable.**
   ...
   LANGUAGE.md:798-799 that reads like boilerplate. That sentence is the
