...
| TIM-02 | 4549 | `a time called <name> is current time.` is the same operation in declaration form. | emit the declaration form at least sometimes instead of the `Get` form | yes — same assertion as TIM-01 on the declared name | `gen leaf current time shape` | exercised | construct emitted by gen leaf current time shape (spelling/position drawn); the documented result is asserted through a sibling row's check, not its own |
| TIM-03 | 4545 | What either form produces is a `time` value (`'s type` reads `Time (static)`). | declare a time, assert its `type` | yes — `If now's type is not "Time (static)" then, … Exit 95.` | `gen leaf current time shape` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 37/200 |
| TIM-04 | 4554 | Components of a time are read with the `'s` possessive. | any property read | yes — implied by any property assertion | `gen leaf current time shape` | exercised | construct emitted by gen leaf current time shape (spelling/position drawn); the documented result is asserted through a sibling row's check, not its own |
| TIM-05 | 4558 | `hour` is the hour of day, 0–23, a Number. | read `hour` and bound it | yes — `If now's hour is less than 0 or now's hour is greater than 23 then, … Exit 95.` Exactly assertable against `unix` (see TIM-11). | `gen leaf current time shape` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 37/200 |
| TIM-06 | 4559 | `minute` is the minute, 0–59, a Number. | read `minute` and bound it | yes — bound 0–59, and exactly `now's unix modulo 3600 divide 60` | `gen leaf current time shape` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 37/200 |
| TIM-07 | 4560 | `second` is the second, 0–59, a Number. | read `second` and bound it | yes — bound 0–59, and exactly `now's unix modulo 60` | `gen leaf current time shape` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 37/200 |
| TIM-08 | 4561 | `day` is the day of month, 1–31, a Number. | read `day` and bound it | yes — bound 1–31 (an exact assertion needs calendar arithmetic the generator does not have) | `gen leaf current time shape` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 37/200 |
| TIM-09 | 4562 | `month` is the month, 1–12, a Number. | read `month` and bound it | yes — bound 1–12 | `gen leaf current time shape` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 37/200 |
| TIM-10 | 4563 | `year` is the year (e.g. 2026), a Number. | read `year` and bound it below | yes — `If ty is less than 2026 then, … Exit 95.` (the build year is a lower bound the generator knows) | `gen leaf current time shape` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 37/200 |
...
| TIM-13 | 4586, 4590 | `current time's <property>` reads a property with nothing stored. | emit an inline read | yes — assert it against a stored reading taken in the same program | `gen leaf current time shape` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 37/200 |
...
| TIM-16 | 4606 | `Wait <N> second.` and `Wait <N> seconds.` are both accepted. | emit both, varying which | yes — as TIM-15 | `gen leaf timer duration and running` | exercised | construct emitted by gen leaf timer duration and running (spelling/position drawn); the documented result is asserted through a sibling row's check, not its own |
| TIM-17 | 4607 | `Wait <N> millisecond.` and `Wait <N> milliseconds.` are both accepted. | emit both | yes — as TIM-15 | `gen leaf timer duration and running` | exercised | construct emitted by gen leaf timer duration and running (spelling/position drawn); the documented result is asserted through a sibling row's check, not its own |
| TIM-18 | 4608 | `Sleep for <N> seconds.` is accepted. | emit it | yes — as TIM-15 | `gen leaf timer duration and running` | exercised | construct emitted by gen leaf timer duration and running (spelling/position drawn); the documented result is asserted through a sibling row's check, not its own |
| TIM-19 | 4609 | `Sleep for <N> milliseconds.` is accepted. | emit it | yes — as TIM-15 | `gen leaf timer duration and running` | exercised | construct emitted by gen leaf timer duration and running (spelling/position drawn); the documented result is asserted through a sibling row's check, not its own |
...
| TIM-22 | 4618 | `Create a timer called '<name>'.` creates a timer under a quoted multi-word name. | emit the `Create` form with a quoted name | yes — assert `'s running` is false before Start | `gen leaf timer duration and running` | exercised | construct emitted by gen leaf timer duration and running (spelling/position drawn); the documented result is asserted through a sibling row's check, not its own |
| TIM-23 | 4619 | `a timer called <name>.` creates a timer without `Create`. | emit the bare form | yes — same | `gen leaf timer duration and running` | exercised | construct emitted by gen leaf timer duration and running (spelling/position drawn); the documented result is asserted through a sibling row's check, not its own |
...
| TIM-25 | 4631 | `Begin` is an accepted spelling of `Start`. | pick between the two spellings at random | yes — `running` is true after `Begin` | `gen leaf timer duration and running` | exercised | construct emitted by gen leaf timer duration and running (spelling/position drawn); the documented result is asserted through a sibling row's check, not its own |
| TIM-26 | 4632 | `Finish` is an accepted spelling of `Stop`. | pick between the two spellings at random | yes — `running` is false after `Finish` | `gen leaf timer duration and running` | exercised | construct emitted by gen leaf timer duration and running (spelling/position drawn); the documented result is asserted through a sibling row's check, not its own |
...
| TIM-33 | 4647 | `start time` is when the timer was started, as a unix timestamp Number. | read it after Start | yes — assert it is >= a `unix` read taken just before Start | `gen leaf timer duration and running` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 12/200 |
| TIM-34 | 4648 | `end time` is when the timer was stopped, as a unix timestamp Number. | read it after Stop | yes — assert `end time >= start time` | `gen leaf timer duration and running` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 12/200 |
| TIM-35 | 4649 | `running` says whether the timer is currently running, as a Boolean. | read it before Start, between, and after Stop | yes — false/true/false | `gen leaf timer duration and running` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 36/200 |
| TIM-36 | 4653 | `in <unit>` casts a duration to a specific unit. | emit the cast | yes — the two units must agree: `milliseconds` within 1000 of `seconds × 1000` | `gen leaf timer duration and running` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 12/200 |
| TIM-37 | 4656 | `<timer>'s duration in seconds` reads. | emit it | yes — assert against the known waits | `gen leaf timer duration and running` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 12/200 |
| TIM-38 | 4657 | `<timer>'s duration in milliseconds` reads. | emit it | yes — same | `gen leaf timer duration and running` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 12/200 |
...
| TIM-42 | 4695–4697 | `Print "{now's hour:02}:{now's minute:02}:{now's second:02}"` prints a `09:05:03`-shaped time. | emit all three slots in one string | yes — the rendered text is 8 characters with `:` at positions 3 and 6 | `gen leaf formatted clock direct` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 69/200 |
| TIM-43 | 4703–4709 | The named-parts variant — each padded property stored in a `text` first, then combined in one more format string — produces the same thing. | emit the two-stage form | yes — same assertion as TIM-42 | `gen leaf formatted clock staged` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 66/200 |
...
| TIM-49 | 4606–4609 *(undocumented)* | *(gap)* `for` is optional after both verbs: `Sleep 4 milliseconds.` and `Wait for 5 milliseconds.` both compile. | vary whether `for` appears | yes — as TIM-15 | `gen leaf timer duration and running` | exercised | construct emitted by gen leaf timer duration and running (spelling/position drawn); the documented result is asserted through a sibling row's check, not its own |
...
| TIM-54 | 4545 *(undocumented)* | *(gap)* A `time` printed, or put in a format slot, renders as its unix second count — a `time` **is** its unix seconds. | print a time and its `unix` and compare | yes — `If "{now}" is not "{now's unix}" then, … Exit 95.` | `gen leaf current time renders as unix` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 38/200 |
