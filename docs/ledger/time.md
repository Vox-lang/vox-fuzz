...
|---|---|---|---|---|---|---|---|
| TIM-01 | 4571, 4574 | `Get current time into <name>.` reads the current date/time into a new name as a `time` value. | emit the statement and read a property back | yes — `If now's unix is less than 1750000000 then, Print "ASSERT TIM-01: expected an epoch second count got {now's unix}". Exit 95.` | `gen leaf current time shape` | exercised | construct emitted by gen leaf current time shape (spelling/position drawn); the documented result is asserted through a sibling row's check, not its own |
| TIM-02 | 4575 | `a time called <name> is current time.` is the same operation in declaration form. | emit the declaration form at least sometimes instead of the `Get` form | yes — same assertion as TIM-01 on the declared name | `gen leaf current time shape` | exercised | construct emitted by gen leaf current time shape (spelling/position drawn); the documented result is asserted through a sibling row's check, not its own |
| TIM-03 | 4571 | What either form produces is a `time` value (`'s type` reads `Time (static)`). | declare a time, assert its `type` | yes — `If now's type is not "Time (static)" then, … Exit 95.` | `gen leaf current time shape` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 37/200 |
| TIM-04 | 4580 | Components of a time are read with the `'s` possessive. | any property read | yes — implied by any property assertion | `gen leaf current time shape` | exercised | construct emitted by gen leaf current time shape (spelling/position drawn); the documented result is asserted through a sibling row's check, not its own |
| TIM-05 | 4584 | `hour` is the hour of day, 0–23, a Number. | read `hour` and bound it | yes — `If now's hour is less than 0 or now's hour is greater than 23 then, … Exit 95.` Exactly assertable against `unix` (see TIM-11). | `gen leaf current time shape` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 37/200 |
| TIM-06 | 4585 | `minute` is the minute, 0–59, a Number. | read `minute` and bound it | yes — bound 0–59, and exactly `now's unix modulo 3600 divide 60` | `gen leaf current time shape` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 37/200 |
| TIM-07 | 4586 | `second` is the second, 0–59, a Number. | read `second` and bound it | yes — bound 0–59, and exactly `now's unix modulo 60` | `gen leaf current time shape` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 37/200 |
| TIM-08 | 4587 | `day` is the day of month, 1–31, a Number. | read `day` and bound it | yes — bound 1–31 (an exact assertion needs calendar arithmetic the generator does not have) | `gen leaf current time shape` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 37/200 |
| TIM-09 | 4588 | `month` is the month, 1–12, a Number. | read `month` and bound it | yes — bound 1–12 | `gen leaf current time shape` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 37/200 |
| TIM-10 | 4589 | `year` is the year (e.g. 2026), a Number. | read `year` and bound it below | yes — `If ty is less than 2026 then, … Exit 95.` (the build year is a lower bound the generator knows) | `gen leaf current time shape` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 37/200 |
| TIM-11 | 4590 | `unix` is the Unix timestamp: seconds since the epoch. | read `unix` and check it is an epoch second count | yes, and **exactly** — `hour`/`minute`/`second` must equal the UTC decomposition of `unix`; that is a real oracle with no reference clock | `gen leaf current time shape` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 74/200 |
| TIM-12 | 4594–4607 | The Time Properties worked example compiles and runs, including the `Print the <name>'s <property>.` article form. | emit the whole sequence, article form included | yes — as the sum of TIM-05…TIM-11 | none — no leaf emits `the <time>'s <property>` with the article, nor `minute`/`second`/`day`/`month` at all | todo | |
| TIM-13 | 4612, 4616 | `current time's <property>` reads a property with nothing stored. | emit an inline read | yes — assert it against a stored reading taken in the same program | `gen leaf current time shape` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 37/200 |
| TIM-14 | 4615–4617 | The Inline Time Access example compiles and runs. | emit the three lines | yes — as TIM-13 | none | todo | |
| TIM-15 | 4622 | `Wait`/`Sleep` pause execution for the specified duration. | emit a wait between a timer's Start and Stop, assert the measured duration | yes — `If measured is less than requested then, … Exit 95.` The generator picks the duration, so it knows the floor. | `gen leaf timer duration and running` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 24/200 |
| TIM-16 | 4632 | `Wait <N> second.` and `Wait <N> seconds.` are both accepted. | emit both, varying which | yes — as TIM-15 | `gen leaf timer duration and running` | exercised | construct emitted by gen leaf timer duration and running (spelling/position drawn); the documented result is asserted through a sibling row's check, not its own |
| TIM-17 | 4633 | `Wait <N> millisecond.` and `Wait <N> milliseconds.` are both accepted. | emit both | yes — as TIM-15 | `gen leaf timer duration and running` | exercised | construct emitted by gen leaf timer duration and running (spelling/position drawn); the documented result is asserted through a sibling row's check, not its own |
| TIM-18 | 4634 | `Sleep for <N> seconds.` is accepted. | emit it | yes — as TIM-15 | `gen leaf timer duration and running` | exercised | construct emitted by gen leaf timer duration and running (spelling/position drawn); the documented result is asserted through a sibling row's check, not its own |
| TIM-19 | 4635 | `Sleep for <N> milliseconds.` is accepted. | emit it | yes — as TIM-15 | `gen leaf timer duration and running` | exercised | construct emitted by gen leaf timer duration and running (spelling/position drawn); the documented result is asserted through a sibling row's check, not its own |
| TIM-20 | 4625–4628 | The four Sleep/Wait example lines compile and run. | emit all four spellings in one program | yes — as TIM-15 | none | todo | |
| TIM-21 | 4639 | A timer is a stopwatch: it tracks a start time, an end time, and the elapsed duration. | Start, do known work, Stop, read all three | yes — `start time <= end time`, and duration within the bounds of the known work | `gen leaf timer and clock` — Start/Stop and `elapsed`, but never `start time` or `end time` | exercised (partly — two of the three properties are never read) | |
| TIM-22 | 4644 | `Create a timer called '<name>'.` creates a timer under a quoted multi-word name. | emit the `Create` form with a quoted name | yes — assert `'s running` is false before Start | `gen leaf timer duration and running` | exercised | construct emitted by gen leaf timer duration and running (spelling/position drawn); the documented result is asserted through a sibling row's check, not its own |
| TIM-23 | 4645 | `a timer called <name>.` creates a timer without `Create`. | emit the bare form | yes — same | `gen leaf timer duration and running` | exercised | construct emitted by gen leaf timer duration and running (spelling/position drawn); the documented result is asserted through a sibling row's check, not its own |
| TIM-24 | 4651, 4653 | `Start the <timer>.` starts it and `Stop the <timer>.` stops it. | emit both, with the article | yes — `running` is true between them and false after | `gen leaf timer duration and running` | exercised | construct emitted by gen leaf timer duration and running (spelling/position drawn); the documented result is asserted through a sibling row's check, not its own |
| TIM-25 | 4657 | `Begin` is an accepted spelling of `Start`. | pick between the two spellings at random | yes — `running` is true after `Begin` | `gen leaf timer duration and running` | exercised | construct emitted by gen leaf timer duration and running (spelling/position drawn); the documented result is asserted through a sibling row's check, not its own |
| TIM-26 | 4658 | `Finish` is an accepted spelling of `Stop`. | pick between the two spellings at random | yes — `running` is false after `Finish` | `gen leaf timer duration and running` | exercised | construct emitted by gen leaf timer duration and running (spelling/position drawn); the documented result is asserted through a sibling row's check, not its own |
| TIM-27 | 4660–4662 | The four words are contextual, not reserved: they open a timer statement only when a name operand follows — `Start the t.`, `stop t.` | emit the lowercase and the article-less spellings too | yes — `running` behaves the same whichever spelling is used | `gen leaf timer duration and running` | exercised | construct emitted by gen leaf timer duration and running (spelling/position drawn); the documented result is asserted through a sibling row's check, not its own |
| TIM-28 | 4662–4663 | `a number called stop is 0.` compiles — the timer words are ordinary names elsewhere. | emit a variable named `stop`/`start`/`begin`/`finish` | yes — assert the variable's value round-trips | none — the generator's name pool never produces these | todo | |
| TIM-29 | 4663 | A program may define and call its own zero-argument `start.` function. | emit `To start.` and a call to it | yes — assert the function's side effect happened | none | todo | |
| TIM-30 | 4664–4665 | "`End` is not a Stop spelling: `end` belongs to the `exit` family of keywords and remains reserved." | — | the first half is right; the second is **false** — `a number called end is 7.` compiles and prints 7, and `End.` is not an exit statement either | none | todo — **blocked on D1** | blocked on D1 |
| TIM-31 | 4671 | `duration` is the total duration of the measured interval. | measure a known interval, assert the total | yes — the generator picks the waits, so it knows the sum | `gen leaf timer duration and running` | exercised | construct emitted by gen leaf timer duration and running (spelling/position drawn); the documented result is asserted through a sibling row's check, not its own |
| TIM-32 | 4672 | `elapsed` is the time elapsed **while the timer is running**. | read `elapsed` without stopping the timer, twice, and assert it grew | yes — `If second read is less than first read then, … Exit 95.` | `gen leaf timer elapsed while running` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 90/200 |
| TIM-33 | 4673 | `start time` is when the timer was started, as a unix timestamp Number. | read it after Start | yes — assert it is >= a `unix` read taken just before Start | `gen leaf timer duration and running` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 12/200 |
| TIM-34 | 4674 | `end time` is when the timer was stopped, as a unix timestamp Number. | read it after Stop | yes — assert `end time >= start time` | `gen leaf timer duration and running` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 12/200 |
| TIM-35 | 4675 | `running` says whether the timer is currently running, as a Boolean. | read it before Start, between, and after Stop | yes — false/true/false | `gen leaf timer duration and running` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 36/200 |
| TIM-36 | 4679 | `in <unit>` casts a duration to a specific unit. | emit the cast | yes — the two units must agree: `milliseconds` within 1000 of `seconds × 1000` | `gen leaf timer duration and running` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 12/200 |
| TIM-37 | 4682 | `<timer>'s duration in seconds` reads. | emit it | yes — assert against the known waits | `gen leaf timer duration and running` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 12/200 |
| TIM-38 | 4683 | `<timer>'s duration in milliseconds` reads. | emit it | yes — same | `gen leaf timer duration and running` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 12/200 |
| TIM-39 | 4684 | `<timer>'s elapsed in seconds` reads. | emit it | yes — same | none — only the `in milliseconds` cast is ever emitted | todo | |
| TIM-40 | 4690–4712 | The Complete Timer Example compiles and behaves as shown: create, start, wait, read `elapsed`, wait, stop, read `duration`, read both raw timestamps. | emit the whole sequence | yes — as the sum of TIM-15, TIM-21, TIM-31, TIM-33, TIM-34, TIM-37 | none | todo (composite) | |
| TIM-41 | 4716–4718 | A time property can be read directly inside a format slot, and the zero-pad specifier applies to it. | emit `"{now's hour:02}"` | yes, and **already the right shape**: a zero-padded hour is two characters whatever the clock reads | `gen leaf format types` (`gen_text.vox:491–493`) — builds the text, copies it into a buffer and checks `'s length is equal to 2`, printing the verdict instead of exiting 95 | exercised (one property; the check is a printed verdict, not an assertion) | |
| TIM-42 | 4721–4723 | `Print "{now's hour:02}:{now's minute:02}:{now's second:02}"` prints a `09:05:03`-shaped time. | emit all three slots in one string | yes — the rendered text is 8 characters with `:` at positions 3 and 6 | `gen leaf formatted clock direct` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 69/200 |
| TIM-43 | 4729–4735 | The named-parts variant — each padded property stored in a `text` first, then combined in one more format string — produces the same thing. | emit the two-stage form | yes — same assertion as TIM-42 | `gen leaf formatted clock staged` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 66/200 |
| TIM-44 | 4582–4590 *(undocumented)* | *(gap)* The time components are **UTC**. The manual names no timezone; the properties agree with the UTC decomposition of `unix` and do not move when `TZ` does. | assert `hour` equals `unix modulo 86400 divide 3600` | yes — exactly, and it is the strongest oracle in the section | `gen leaf current time utc agreement` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 63/200 |
| TIM-45 | 4671–4672 *(undocumented)* | *(gap)* The "requires cast" note is not enforced: `duration` and `elapsed` read with no cast, and the uncast reading is the seconds one. | read both forms and compare | yes — `If uncast is not (duration in seconds) then, … Exit 95.` | `gen leaf timer uncast agreement` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 42/200 |
| TIM-46 | 4667–4675 *(undocumented)* | *(gap)* A timer that was never started reads 0 for every property and is not running; `Stop` before any `Start` is not an error, but leaves a duration equal to the whole reading of the monotonic clock (the machine's uptime), not to any interval the program asked for. | emit Stop-before-Start and read the properties back | partly — "all zero before any Stop" is exactly assertable; the post-Stop duration is a machine-dependent number, so only "absurdly larger than anything this program waited for" is assertable | `gen leaf timer never started` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 126/200 |
| TIM-47 | 4651 *(undocumented)* | *(gap)* Starting an already-run timer **restarts** it: the next `duration` covers only the second run. | Start/Stop twice with different waits, assert the second duration is the shorter one | yes — the generator picks both waits | `gen leaf timer restart resets` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 87/200 |
| TIM-48 | 4672 *(undocumented)* | *(gap)* After `Stop`, `elapsed` freezes and reads the same as `duration`. | read `elapsed` twice after Stop with a wait between | yes — `If second read is not first read then, … Exit 95.` and `elapsed == duration` | `gen leaf timer and clock` emits the shape (it reads `elapsed` after `Stop`) but asserts nothing about it beyond "not negative", printed | exercised | |
| TIM-49 | 4632–4635 *(undocumented)* | *(gap)* `for` is optional after both verbs: `Sleep 4 milliseconds.` and `Wait for 5 milliseconds.` both compile. | vary whether `for` appears | yes — as TIM-15 | `gen leaf timer duration and running` | exercised | construct emitted by gen leaf timer duration and running (spelling/position drawn); the documented result is asserted through a sibling row's check, not its own |
| TIM-50 | 4631–4635 *(undocumented)* | *(gap)* The duration operand is a literal or a bare variable; an expression there is a compile error naming the four unit words. | — | **no** — a compile-error claim; emitting it would produce a non-compiling program, outside the generator's contract. It is a **constraint on the generator**: compute the duration into a name first. | none | not assertable | |
| TIM-51 | 4632–4635 *(undocumented)* | *(gap)* A negative duration in **seconds** returns immediately (the kernel rejects a negative `tv_sec`). | emit a negative seconds wait, assert the program continued quickly | yes — measure it with a timer | `gen leaf timer negative seconds wait` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 51/200 |
| TIM-52 | 4637–4675 *(undocumented)* | *(gap)* Accepted spellings the manual never lists: `retrieve`/`fetch` for `get`, `stopwatch` for `timer`, `pause` for `wait`, `delay` for `sleep`, `timestamp`/`unixtime` for `unix`, and plural `hours`/`minutes`/`days`/`months`/`years` for the property names. | vary the spelling wherever more than one exists | yes — each alias must give the same answer as its canonical form | none — every leaf uses one fixed spelling | todo — see **Discrepancy 4** | |
| TIM-53 | 4586 *(undocumented)* | *(gap)* The plural spellings stop short of `seconds`: `now's second` reads, `now's seconds` is a compile error, because `seconds` is the Wait unit word. | — | **no** — a compile-error claim; it is a constraint on TIM-52's spelling variation | none | not assertable | |
| TIM-54 | 4571 *(undocumented)* | *(gap)* A `time` printed, or put in a format slot, renders as its unix second count — a `time` **is** its unix seconds. | print a time and its `unix` and compare | yes — `If "{now}" is not "{now's unix}" then, … Exit 95.` | `gen leaf current time renders as unix` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 38/200 |
| TIM-55 | 4582–4675 *(undocumented)* | *(gap)* The section's property words are reserved variable names: `duration`, `elapsed`, `running`, `timer`, `stopwatch`, `time`, `unix`, `hour`, `minute`, `day`, `month`, `year`, `wait`, `sleep`, `milliseconds`, `ms`, `current` are all rejected; `second`, `start`, `begin`, `stop`, `finish`, `end` are accepted. | — | **no** — a naming rule, not a runtime result. It is a **constraint on the generator's name pool**, and the six accepted words are names it is allowed to use (TIM-28). | n/a | not assertable | |
| TIM-56 | 4651 *(undocumented)* | *(gap)* `Start`/`Stop` on a non-timer is `Start requires a timer: <name>`, and on an unknown name is `Unknown timer: <name>`. | — | **no** — compile-error claims. Recorded because the diagnostic's span points at the **declaration** of the operand, not at the offending `Start` — which will mislead triage of a generated program that fails to compile. | none | not assertable | |
| TIM-57 | 4582–4590 *(undocumented)* | *(gap)* A time property read from a plain number is accepted and the number is decomposed as an epoch second count (`3` → unix 3, hour 0, year 1970). | emit a time property on a non-time number | yes — `If (3)'s year is not 1970 then, … Exit 95.` — and it is a memory-safety row: it must not crash | `gen leaf time property from number` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 93/200 |
| TIM-58 | 4679 *(undocumented)* | *(gap)* `in` accepts only `second`/`seconds`/`millisecond`/`milliseconds`/`ms`. `minutes`, `hours` and `nanoseconds` are compile errors. | — | **no** — a compile-error claim; it is the constraint that bounds TIM-36's unit variation | none | not assertable | |
| TIM-59 | 4582–4590 *(undocumented)* | *(gap)* A time property read from a **text** is a compile error: `Property 'year' requires a time value (number)`. | — | **no** — a compile-error claim. Its parenthetical is the clearest statement anywhere that a `time` is a number. | none | not assertable | |
| TIM-60 | 4639, 4673–4674 *(undocumented)* | *(gap)* `duration`/`elapsed` and `start time`/`end time` come from **different clocks**: the timestamps are unix wall-clock seconds, the durations are monotonic and carry millisecond precision. A 1.5-second job shows start and end one second apart and a duration of 1500 ms. | read all four around a known interval | yes — `duration in milliseconds` must be finer than the timestamp difference allows | none | todo | |

### A note on the worked examples

All five run, but none of them lays out the way it reads. `Print` ends
its line (LANGUAGE.md:3374–3387 gives `without newline` as the way to
avoid that), so
...
LANGUAGE.md:4664–4665, closing the note on contextual timer words:
...
The manual gives no behaviour for a negative duration (LANGUAGE.md:4631–
...
LANGUAGE.md:5191–5266 introduces its table as the alternate spellings
...
LANGUAGE.md:4582–4590 gives `hour` as "Hour of day (0-23)" and names no
...
- the property names are exactly `hour`, `minute`, `second`, `day`,
  ...
  property of either — LANGUAGE.md:4582–4590, 4669–4675, TIM-05…TIM-11,
  TIM-31…TIM-35
- `in` is followed only by a second or millisecond unit word —
  LANGUAGE.md:4679, TIM-36, TIM-58
