...
this section's — it belongs to Functions / Variables.
...
Pro-compiler reading: the function body is analysed before the
declaration is reached, so at that point `label` has no known type and the
printer falls through to the number path. That is a forward-reference
ordering rule the manual never states, not a wrong answer for a
well-formed program — and Vox is otherwise a single-pass, declare-before-
use language. Under that reading LANGUAGE.md:705 needs an ordering
sentence and the compiler needs a diagnostic where it currently prints
garbage. **Not filed.**
