...
Source: `../vox/LANGUAGE.md` lines 5480–5520, Vox 0.4.10 (5545 lines, vox
527cb89), confirmed 2026-08-22 (Compiler Usage: Basic Usage, Options,
...
LANGUAGE.md:5495 documents `--link <libs>` as "link against shared
libraries (comma-separated)," and the worked example at :5303 pairs it
with `--lib-path`: `vox main.vox --link math --lib-path ./libs`. Minimal
repro (`D1.vox`; full four-transcript version in `CLI-05.vox`):

**Update, 2026-08-22.** The manual was reworded in vox commit `70ca1c2`
(LANGUAGE.md:5495, and a new paragraph at :5262–5265 introducing the
`--link` section): `--link` is now documented as needed only for a `.so`
with no `.lib` (foreign or hand-built); a Vox library with a `.lib` is
said to be linked by `see`ing it instead. That is exactly the "reading
in which the compiler is correct" this discrepancy already proposed
below — the manual now says it explicitly rather than leaving it to be
inferred. This does not fully resolve the finding: the Compiler Usage
section's own example at :5303 still pairs `--link` with a `see`-based
consumer shape (`main.vox`), the same ambiguity noted below, though its
comment was reworded to "a `.so` that has no `.lib`" (:5296–5303) —
closer to the new reading but still not a worked example that actually
lacks a `.lib`. Not filed; re-verify D1's transcripts still hold before
next touching this row (not re-run in this pass — the compiler's `--link`
handling itself did not change today, only the prose around it).
