...
| VAL-10 | 2741–2745 | Worked retype example: `numstr` is `"357"`; `numstr is a number.`; `print numstr add 1` prints `358` (arithmetic works because the variable is now tracked as a number). | reproduce, assert `358` | yes — exact value known at generation time | none | **verified** — `gen leaf value retype sum` asserts post-retype arithmetic works (the 357 add 1 shape) (value band live 2026-08-23) | `gen leaf value retype sum` (ASSERT VAL-10) · vox 0.4.11 · seeds 41000–41219, 77777–77788 |
