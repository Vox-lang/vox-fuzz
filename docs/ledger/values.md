...
manual has **"Conditional `value` returns work"** (LANGUAGE.md:2860–2881)
...
been clarified at LANGUAGE.md:2802–2805 to say so explicitly: "The
...
- `nothing` always prints as the literal word `nothing` (never `null`,
  `nil`, empty, or `0`) — LANGUAGE.md:2895, VAL-21 / VAL-22
- `null`, `nil`, and `nothing` are interchangeable as the absent-value
  ...
  lists all three as valid (LANGUAGE.md:2909), so a leaf that *always*
...
- arithmetic on a literal `nothing` is a compile error, so a generated
  ...
  expression — LANGUAGE.md:2942, VAL-28
- bare arithmetic on a `value` is a compile error, so a generated program
  ...
  it (`v is a number.`) — LANGUAGE.md:2777, VAL-08 / VAL-09
- ~~the conditional `value` return (factorial pattern, no `Return` on the
  ...
  on the `To` line — LANGUAGE.md:2860–2881, VAL-19 — WITHDRAWN, see below~~
  **Withdrawn 2026-08-21: this invariant is no longer justified.** vox #43
  (0.4.8) made the conditional form work and LANGUAGE.md:2860–2881 now
  ...
  return type (LANGUAGE.md:2883–2889, VAL-32), so a leaf must declare the same
