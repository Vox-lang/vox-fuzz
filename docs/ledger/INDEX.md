...
| KEY | `keywords.md` | 5033–5341 | Keywords: articles, starters, flag schema, connectors, `and`, reserved aliases, two classes of special word, contextual keywords | **yes** | 88 | 36 | 0 | 4 open, 4 resolved: D4 **RESOLVED, vox #106, 0.4.14 aliases half + 0.4.15 Statement Starters half**; D5/D6/D7 **RESOLVED, vox #56** (`all the numbers from A to B` bugs — dropped end bound, segfault, wrong-format print); D1 thirteen Statement-Starters words aren't reserved as names; D2 `fork` is declarable and reading it forks the process; D3 `reap` is refused, but not with the documented diagnostic; D8 a missing required flag exits 1 silently — **STILL OPEN, design question for Josj**, no fix number exists | |
