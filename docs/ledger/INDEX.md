...
| FLW | `control-flow.md` | 2188–2361 | Control flow: if, while, for each, repeat, loop control, termination, increment/decrement | **yes** | 45 | 17 | 0 | 3 (D1 the manual's usage-guard example doesn't guard; D2 `Increment` is a no-op on `float`, a compile error on `text`; D3 the loop variable outlives its loop) | |
