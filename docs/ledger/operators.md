...
BUF-04 "unobservable from inside Vox" kind. Six rows nevertheless carry
the status `not assertable` (OPR-22, OPR-23, OPR-34, OPR-40, OPR-41,
OPR-42): their outcome is a **compile error**, which no runtime leaf can
be built to carry, so the probe records the diagnostic and the harness —
not a generated program — is what would notice a change. Three of those
six are rows where the manual's claim is **contradicted** by the
compiler. PROCEDURE.md's status ladder has no value for that, so it is
said in the row rather than invented as a new status, the way
...
- `times` never appears in an `append … to …` value slot — again no
  manual citation; OPR-41, a known open finding. The generator already
  enforces this in `gen append expr`, which means **`src/gen_core.vox`
  is currently enforcing an undeclared rule** — legitimately, since the
  alternative is emitting programs that do not compile, but it is on the
  list and it should be written down rather than left in a comment.
...
**Assertable: 36 of 42.** The six that are not are the six whose
documented or observed outcome is a **compile error** (OPR-22, OPR-23,
OPR-34, OPR-40, OPR-41, OPR-42) — a runtime leaf cannot carry them, and
their probes record the diagnostic instead. Nothing in this section is
unobservable in the BUF-04 sense; there are no implementation-detail
rows.

**Coverage: 17 exercised, 0 verified, 19 todo, 6 not assertable.** Exercised is generous —
