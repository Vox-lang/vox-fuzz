...
| KEY-02 | 5040 | `the` references an already-declared variable. | emit `the x` as a plain read and as an assignment target | yes — same reasoning | `gen leaf article the reference` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 11/200 |
...
| KEY-49 | 5239 | `message` is a reserved alias of the type name `text`. | emit `a message called … is "…"` sometimes instead of `a text called` | yes — the declared variable behaves as a text and reports `Text (static)` | `gen leaf reserved alias text types` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 32/200 |
| KEY-50 | 5240 | `string` is a reserved alias of the type name `text`. | same, third spelling | yes | `gen leaf reserved alias text types` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 32/200 |
...
| KEY-53 | 5310–5313 | `length` is no longer a reserved alias — `a number called length is 1.` compiles. | emit a variable actually **named** `length` | yes — it reads back as what was assigned | `gen leaf contextual keyword length` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 17/200 |
| KEY-54 | 5310 | `x's length` still means the same as `x's size`. | emit both accessors on the same object and compare | yes — `If payload's length is not payload's size then, Exit 95.` | `gen leaf contextual keyword length` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 34/200 |
...
(LANGUAGE.md:5044–5082, KEY-82). `each` and `without` were added to the
