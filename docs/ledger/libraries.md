...
THG2-49 through THG2-58 rows, LANGUAGE.md:1682–1758) already; they are
...
LANGUAGE.md:5013–5016: "It will not overwrite a `.lib` that already
...
LANGUAGE.md:4897 spells the second retired form as `see "lib" version
...
LANGUAGE.md:5019–5025 shows the `.lib` format worked example with the
...
Nothing about the `.lib` grammar (LANGUAGE.md:5033–5039) suggests two
...
LANGUAGE.md:5051–5053 says "No returning clause means the function
returns nothing," and the six-step `see`-of-`.lib` process
(LANGUAGE.md:5069–5077, step 5) says calls "type-check like any other
...
library declares exactly one `Library` block (LANGUAGE.md:5027–5029
...
1. A repeat `--shared` build silently overwrites an existing `.lib`
   instead of erroring, contradicting LANGUAGE.md:5013–5016 outright —
