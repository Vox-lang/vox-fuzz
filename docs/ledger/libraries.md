...
Source: `../vox/LANGUAGE.md` lines **4791–5136** (`## Libraries and Imports`
through the `---` before `## Compiler Usage`), manual version **0.4.8**,
vox commit `b26f66e`. `INDEX.md`'s pinned range for LIB (4663–5009) is
stale — it predates a manual re-pin that moved everything after
`## Examples`. The range above is re-confirmed at b26f66e: line 4791 is
`## Libraries and Imports` and line 5138 is `## Compiler Usage`.
...
THG2-49 through THG2-58 rows, LANGUAGE.md:1595–1671) already; they are
...
### 1. A repeat `--shared` build silently overwrites an existing `.lib`, including one hand-edited

LANGUAGE.md:4926–4929: "It will not overwrite a `.lib` that already
exists — a repeat `--shared` build errors and asks you to remove the
`.lib` first, then regenerates the `.so` and `.lib` together, so a
rebuild cannot silently clobber an interface you have pinned or edited."
Repro (`D1.vox`, fuller version in `shared/LIB-overwrite.vox`):
...
from scratch, discarding the pinned line — exactly the harm the
paragraph says is prevented.

**The reading in which the compiler is right:** the weakest of this
ledger's discrepancies to defend. The only reading available is that the
protection might have been meant to apply only when a *different*
library's `.lib` already occupies the path (a genuine identity
collision), not a rebuild of the same library at the same output stem —
but that reading does not survive the repro either, since `d1.lib` here
*is* the same library's own `.lib`, deliberately edited, and it still
gets clobbered. This looks like described-but-unimplemented behavior
rather than an ambiguous sentence. Not filed.

### 2. Manual's own second retired-syntax example does not parse as written

LANGUAGE.md:4810 spells the second retired form as `see "lib" version
...
### 3. The `.lib` worked example's zero-parameter entry omits `To`; the real compiler never does

LANGUAGE.md:4931–4938 shows the `.lib` format worked example with the
`greet` entry (a zero-parameter, no-return function) written as bare
`greet.`, while the `'add two numbers'` entry above it is written `To
'add two numbers' with ...`. Repro (`shared/D3.vox`):
...
**The reading in which the compiler is right:** this reads as a plain
transcription slip in the manual's worked example — dropping `To` for
the one entry that happens to need no `with` clause — rather than a
second, undocumented spelling the compiler is supposed to accept.
Nothing about the `.lib` grammar (LANGUAGE.md:4946–4952) suggests two
valid spellings for a zero-parameter entry, and every other worked
`.lib` example in this ledger (`shared/LIB-types.vox`,
`shared/LIB-barereturn.vox`) is consistently `To`-prefixed. A one-word
manual fix.

### 4. A `.lib` entry with no `, returning` clause is not type-checked as "returns nothing" at the call site

LANGUAGE.md:4964–4966 says "No returning clause means the function
returns nothing," and the six-step `see`-of-`.lib` process
(LANGUAGE.md:4982–4990, step 5) says calls "type-check like any other
...
library declares exactly one `Library` block (LANGUAGE.md:4940–4942
...
**Four discrepancies**, none filed:
1. A repeat `--shared` build silently overwrites an existing `.lib`
   instead of erroring, contradicting LANGUAGE.md:4926–4929 outright —
   the weakest-defended of the four, most likely simply unimplemented.
...
3. The `.lib`-format worked example spells a zero-parameter entry
   without `To`; the compiler always writes `To`. A one-word manual fix.
4. **The most consequential one.** A `.lib` entry with no `, returning`
