...
**Resolution (2026-08-22):** the manual was corrected in vox commit
`70ca1c2` — the worked example at LANGUAGE.md:5027–5033 now shows
...
LANGUAGE.md:5027–5033) with the `greet` entry (a zero-parameter,
...
Nothing about the `.lib` grammar (LANGUAGE.md:5041–5047) suggests two
...
LANGUAGE.md:5061–5063 says "No returning clause means the function
returns nothing," and the six-step `see`-of-`.lib` process
(LANGUAGE.md:5088–5096, step 5) says calls "type-check like any other
...
library declares exactly one `Library` block (LANGUAGE.md:5035–5037
...
1. **RESOLVED.** A repeat `--shared` build silently overwrote an
   ...
   overwrite (LANGUAGE.md:5021–5024) instead of a protection the
...
3. **RESOLVED.** The `.lib`-format worked example spelled a
   ...
   (LANGUAGE.md:5027–5033), matching what the compiler always wrote.
