...
LANGUAGE.md:2429–2432 (old: 2238–2243):
...
**Strongest reading under which the compiler is correct:** LANGUAGE.md:710–711
...
**Resolution confirmed, 2026-08-22.** LANGUAGE.md:2582–2596 now states the corrected rule verbatim and shows exactly this idiom as the worked example (2460–2465). Re-verified: `entry's "tag"` inside the loop is still, correctly, a compile error.
...
**Resolution confirmed, 2026-08-22.** LANGUAGE.md:2634–2652 now states the "does not narrow" rule explicitly and shows the index-loop idiom verbatim (2513–2522), matching the lawyer's proposed fix exactly.
...
**Resolution confirmed, 2026-08-22.** LANGUAGE.md:2654–2659 now explicitly says the cast is refused for the same reason and points at the index-loop idiom instead. Re-verified: `item as a number` on a mixed element still fails to compile, as expected — it is documented as failing, not as working.
...
not like a crash. That is precisely the failure mode LANGUAGE.md:718–721 says
...
**Resolution confirmed, 2026-08-22.** LANGUAGE.md:2493–2494 now reads "(prints: [[...]] then cyclic — abbreviated: 64 opening brackets, then `...`, then 64 closing brackets, then `cyclic`)" — the word "abbreviated" is there.
...
LANGUAGE.md:3423–3425 (was 3082–3085): all format-string sinks "share one name resolver, so
special names … render **identically** whether the result is printed, written
to a file, or built into a buffer". LANGUAGE.md:3399 (was 3054–3056): a format string used
...
prefixes), not every value type; and LANGUAGE.md:2431–2434 already warns that
...
LANGUAGE.md:1947 introduces `lhs is equal to rhs` with no type
restriction, and LANGUAGE.md:4869 lists `is equal to` / `is` as *the*
...
LANGUAGE.md:710–721 was written to eliminate. And whatever the right
...
- list literals always wrapped in `[` `]` with `, ` between elements — LANGUAGE.md:2365, LST-01
- list element indices never 0 (1-indexed) — LANGUAGE.md:2375, LST-02
- an empty list always renders exactly `[]` — LANGUAGE.md:2371, 2377, LST-04
- booleans inside a list always render `1`/`0`, never `true`/`false` — LANGUAGE.md:2396, LST-17
- text elements inside a list always render quoted; numbers never — LANGUAGE.md:2396, LST-09
- nested list elements always render with their own brackets, recursively — LANGUAGE.md:2447–2450, LST-23
- recursive printing always stops at exactly depth 64 and always emits `...` at the cap — LANGUAGE.md:2484, LST-32, LST-54
- map literals always wrapped in `{` `}` with `"key": value` pairs — LANGUAGE.md:2507, LST-37
- an empty map always renders exactly `{}` — LANGUAGE.md:2507, LST-37
- every map key is always a quoted text — LANGUAGE.md:2506, 2598–2599, LST-36, LST-58
- map reads are always spelled `<map>'s "<key>"` — LANGUAGE.md:2517, LST-39
- `keys` and `values` always come back in insertion order — LANGUAGE.md:2541–2542, LST-47
- a replaced key always keeps its original position, never moves to the end — LANGUAGE.md:2535–2537, LST-45, LST-47
- type-predicate nouns always drawn from the closed set `number`/`text`/`decimal`/`boolean`/`list`/`map` plus `integer`/`string`/`float`/`real`/`bool`/`dictionary` — LANGUAGE.md:2618–2619, LST-62, LST-63
