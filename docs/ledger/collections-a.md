...
| LST-04 | 2388 | Empty lists `[]` are allowed. | declare `[]`, assert `length` is 0 and it prints `[]` | yes — `If xs's length is not 0 then, Exit 95.` and `If xs's empty is not 1 then, Exit 95.` | `gen leaf list empty state` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 20/200 |
...
**Resolution confirmed, 2026-08-22.** LANGUAGE.md:2632–2646 now states the corrected rule verbatim and shows exactly this idiom as the worked example (2460–2465). Re-verified: `entry's "tag"` inside the loop is still, correctly, a compile error.
...
**Resolution confirmed, 2026-08-22.** LANGUAGE.md:2684–2702 now states the "does not narrow" rule explicitly and shows the index-loop idiom verbatim (2513–2522), matching the lawyer's proposed fix exactly.
...
**Resolution confirmed, 2026-08-22.** LANGUAGE.md:2704–2709 now explicitly says the cast is refused for the same reason and points at the index-loop idiom instead. Re-verified: `item as a number` on a mixed element still fails to compile, as expected — it is documented as failing, not as working.
...
LANGUAGE.md:3477–3479 (was 3082–3085): all format-string sinks "share one name resolver, so
special names … render **identically** whether the result is printed, written
to a file, or built into a buffer". LANGUAGE.md:3451 (was 3054–3056): a format string used
...
restriction, and LANGUAGE.md:4999 lists `is equal to` / `is` as *the*
...
- every map key is always a quoted text — LANGUAGE.md:2527, 2648–2649, LST-36, LST-58
- map reads are always spelled `<map>'s "<key>"` — LANGUAGE.md:2538, LST-39
- `keys` and `values` always come back in insertion order — LANGUAGE.md:2588–2589, LST-47
- a replaced key always keeps its original position, never moves to the end — LANGUAGE.md:2582–2584, LST-45, LST-47
- type-predicate nouns always drawn from the closed set `number`/`text`/`decimal`/`boolean`/`list`/`map` plus `integer`/`string`/`float`/`real`/`bool`/`dictionary` — LANGUAGE.md:2668–2669, LST-62, LST-63
