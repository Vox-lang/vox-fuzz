...
LANGUAGE.md:2424–2427 (old: 2238–2243):
...
**Resolution confirmed, 2026-08-22.** LANGUAGE.md:2577–2591 now states the corrected rule verbatim and shows exactly this idiom as the worked example (2460–2465). Re-verified: `entry's "tag"` inside the loop is still, correctly, a compile error.
...
**Resolution confirmed, 2026-08-22.** LANGUAGE.md:2629–2647 now states the "does not narrow" rule explicitly and shows the index-loop idiom verbatim (2513–2522), matching the lawyer's proposed fix exactly.
...
**Resolution confirmed, 2026-08-22.** LANGUAGE.md:2649–2654 now explicitly says the cast is refused for the same reason and points at the index-loop idiom instead. Re-verified: `item as a number` on a mixed element still fails to compile, as expected — it is documented as failing, not as working.
...
**Resolution confirmed, 2026-08-22.** LANGUAGE.md:2488–2489 now reads "(prints: [[...]] then cyclic — abbreviated: 64 opening brackets, then `...`, then 64 closing brackets, then `cyclic`)" — the word "abbreviated" is there.
...
LANGUAGE.md:3418–3420 (was 3082–3085): all format-string sinks "share one name resolver, so
special names … render **identically** whether the result is printed, written
to a file, or built into a buffer". LANGUAGE.md:3394 (was 3054–3056): a format string used
...
prefixes), not every value type; and LANGUAGE.md:2426–2429 already warns that
...
restriction, and LANGUAGE.md:4864 lists `is equal to` / `is` as *the*
...
- list literals always wrapped in `[` `]` with `, ` between elements — LANGUAGE.md:2360, LST-01
- list element indices never 0 (1-indexed) — LANGUAGE.md:2370, LST-02
- an empty list always renders exactly `[]` — LANGUAGE.md:2366, 2372, LST-04
- booleans inside a list always render `1`/`0`, never `true`/`false` — LANGUAGE.md:2391, LST-17
- text elements inside a list always render quoted; numbers never — LANGUAGE.md:2391, LST-09
- nested list elements always render with their own brackets, recursively — LANGUAGE.md:2442–2445, LST-23
- recursive printing always stops at exactly depth 64 and always emits `...` at the cap — LANGUAGE.md:2479, LST-32, LST-54
- map literals always wrapped in `{` `}` with `"key": value` pairs — LANGUAGE.md:2502, LST-37
- an empty map always renders exactly `{}` — LANGUAGE.md:2502, LST-37
- every map key is always a quoted text — LANGUAGE.md:2501, 2593–2594, LST-36, LST-58
- map reads are always spelled `<map>'s "<key>"` — LANGUAGE.md:2512, LST-39
- `keys` and `values` always come back in insertion order — LANGUAGE.md:2536–2537, LST-47
- a replaced key always keeps its original position, never moves to the end — LANGUAGE.md:2530–2532, LST-45, LST-47
- type-predicate nouns always drawn from the closed set `number`/`text`/`decimal`/`boolean`/`list`/`map` plus `integer`/`string`/`float`/`real`/`bool`/`dictionary` — LANGUAGE.md:2613–2614, LST-62, LST-63
