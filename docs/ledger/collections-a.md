...
| LST-53 | 2548–2551 | `_map_print` shares the same 64-deep `_print_depth` budget as `_list_print`, so a mixed map/list tree is cycle-safe. | build a map→list→map cycle, print under `on error`, assert the handler fired and the program continued | yes | none | todo — hand-verified (`LST-53.vox`); same memory-safety weight as LST-32 | |
| LST-54 | 2551–2552 | A self-referential map (`set m's "self" to m.`) prints 64 levels deep, then `...`, sets the error flag, and unwinds safely. | reproduce, assert the handler fired and the program continued | yes for the error flag and the survival; the depth is eyeball-only (**Discrepancy 8**) | none | todo — hand-verified (`LST-54.vox`), exactly 64 levels | |
| LST-55 | 2554–2556 | The `is a map` predicate folds to true on a statically-typed map variable and compares the tag at run time on a mixed value. | emit both forms, assert the branch | yes | none — no leaf emits any predicate | todo — hand-verified (`LST-55.vox`) | |
...
LANGUAGE.md:2408–2411 (old: 2238–2243):
...
**Strongest reading under which the compiler is correct:** LANGUAGE.md:694–695
...
**Resolution confirmed, 2026-08-22.** LANGUAGE.md:2561–2575 now states the corrected rule verbatim and shows exactly this idiom as the worked example (2460–2465). Re-verified: `entry's "tag"` inside the loop is still, correctly, a compile error.
...
**Resolution confirmed, 2026-08-22.** LANGUAGE.md:2613–2631 now states the "does not narrow" rule explicitly and shows the index-loop idiom verbatim (2513–2522), matching the lawyer's proposed fix exactly.
...
**Resolution confirmed, 2026-08-22.** LANGUAGE.md:2633–2638 now explicitly says the cast is refused for the same reason and points at the index-loop idiom instead. Re-verified: `item as a number` on a mixed element still fails to compile, as expected — it is documented as failing, not as working.
...
not like a crash. That is precisely the failure mode LANGUAGE.md:702–705 says
...
**Resolution confirmed, 2026-08-22.** LANGUAGE.md:2472–2473 now reads "(prints: [[...]] then cyclic — abbreviated: 64 opening brackets, then `...`, then 64 closing brackets, then `cyclic`)" — the word "abbreviated" is there.
...
prefixes), not every value type; and LANGUAGE.md:2410–2413 already warns that
...
LANGUAGE.md:694–705 was written to eliminate. And whatever the right
...
- list literals always wrapped in `[` `]` with `, ` between elements — LANGUAGE.md:2344, LST-01
- list element indices never 0 (1-indexed) — LANGUAGE.md:2354, LST-02
- an empty list always renders exactly `[]` — LANGUAGE.md:2350, 2356, LST-04
- booleans inside a list always render `1`/`0`, never `true`/`false` — LANGUAGE.md:2375, LST-17
- text elements inside a list always render quoted; numbers never — LANGUAGE.md:2375, LST-09
- nested list elements always render with their own brackets, recursively — LANGUAGE.md:2426–2429, LST-23
- recursive printing always stops at exactly depth 64 and always emits `...` at the cap — LANGUAGE.md:2463, LST-32, LST-54
- map literals always wrapped in `{` `}` with `"key": value` pairs — LANGUAGE.md:2486, LST-37
- an empty map always renders exactly `{}` — LANGUAGE.md:2486, LST-37
- every map key is always a quoted text — LANGUAGE.md:2485, 2577–2578, LST-36, LST-58
- map reads are always spelled `<map>'s "<key>"` — LANGUAGE.md:2496, LST-39
- `keys` and `values` always come back in insertion order — LANGUAGE.md:2520–2521, LST-47
- a replaced key always keeps its original position, never moves to the end — LANGUAGE.md:2514–2516, LST-45, LST-47
- type-predicate nouns always drawn from the closed set `number`/`text`/`decimal`/`boolean`/`list`/`map` plus `integer`/`string`/`float`/`real`/`bool`/`dictionary` — LANGUAGE.md:2597–2598, LST-62, LST-63
