...
| FLW-05 | 2208 | Several actions in one branch are separated by commas. | as FLW-04, specifically in the `Otherwise` arm | yes | none (all existing arms are single-action) | todo | |
...
| FLW-41 | 2351 | `terminate` is an alternative keyword for `Exit`, and takes a code. | as FLW-40 | yes (harness-side) | **none** | todo | |
