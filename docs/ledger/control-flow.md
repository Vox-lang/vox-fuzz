...
of LANGUAGE.md:402–449 and :3014 (no `then`, attaches to a base action);
...
1 is explicit, LANGUAGE.md:252 restates it for `Otherwise`, and the
...
LANGUAGE.md:2298–2300 gives `Increment the counter.` / `Decrement the
...
LANGUAGE.md:2206–2207 introduces the loop variable under the heading
...
- `then,` follows every `If`/`When` condition — LANGUAGE.md:2141, FLW-01
- a comma follows every loop head (`While <cond>,`, `For each … ,`,
  `Repeat <count> times,`) — LANGUAGE.md:2171/2097/2124, FLW-11/15/23
- `times` follows every `Repeat` count — LANGUAGE.md:2225, FLW-23
- `Break`/`Continue` never appear outside a loop — FLW-33 (compile error;
  undocumented but hard-enforced)
- `Exit`'s code, when present, is a number — LANGUAGE.md:2277, FLW-34
...
<action>`, LANGUAGE.md:402). Had I trusted the names, the else-if chain —
