...
| THG2-09 | 1367 | `is not` is the negation of `is`. | same probe, both directions | yes, same assertion inverted | none | todo — hand-verified (covered by `THG2-08.vox`) | |
| THG2-10 | 1367–1368 | The comparison is written out by the compiler, so it recurses into nested things. | two `t3`s differing only in `g1's x1`, assert `is not`; then make them match, assert `is` | yes | none | todo — hand-verified (`THG2-10.vox`) | |
