...
LANGUAGE.md:1290: "A quoted field name prints in the quotes it is written
with." Repro (`D1.vox`):
...
LANGUAGE.md:1675–1676: "The same error names a function, a parameter, a
...
The sibling owner-return diagnostic (LANGUAGE.md:1427–1430) deliberately
...
LANGUAGE.md:1844–1852 documents the default-must-match-the-field-type
check. Repro (`D4.vox`):
...
only (LANGUAGE.md:1791–1793), and the text is exactly right. The
...
1. every free call uses the preposition `of`; `to`, `with` and `on` never
   appear in a call (LANGUAGE.md:1494 says all four introduce arguments)
2. every member call passes its arguments with `with`, and the receiver
   call passes none at all (LANGUAGE.md:1513–1514 permits further
...
4. every thing definition indents its entries by exactly four spaces
   (LANGUAGE.md:1806–1808 says indentation is not required)
