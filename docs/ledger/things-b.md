...
LANGUAGE.md:1260: "A quoted field name prints in the quotes it is written
with." Repro (`D1.vox`):
...
LANGUAGE.md:1645–1646: "The same error names a function, a parameter, a
...
The sibling owner-return diagnostic (LANGUAGE.md:1397–1400) deliberately
...
LANGUAGE.md:1814–1822 documents the default-must-match-the-field-type
check. Repro (`D4.vox`):
...
only (LANGUAGE.md:1761–1763), and the text is exactly right. The
...
1. every free call uses the preposition `of`; `to`, `with` and `on` never
   appear in a call (LANGUAGE.md:1464 says all four introduce arguments)
2. every member call passes its arguments with `with`, and the receiver
   call passes none at all (LANGUAGE.md:1483–1484 permits further
...
4. every thing definition indents its entries by exactly four spaces
   (LANGUAGE.md:1776–1778 says indentation is not required)
