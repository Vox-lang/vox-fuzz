...
Source: `../vox/LANGUAGE.md` lines **1168–1788**, manual version **0.4.8**
— Things § Printing, Equality, The manifest, The three call forms, One
identifier space, Definitions are top-level only, Cross-file definitions,
`.lib` export of a thing is not yet supported, Sentence consumption and
multi-line definitions, Definition diagnostics, Type predicates and the
runtime tag, Design notes for review.

The brief's line range is correct for 0.4.8: 1168 is `### Printing` and
1789 is `## Expressions`. `INDEX.md` still pins **0.4.7** (5112 lines);
the manual is now **0.4.8** (5238 lines), so every other ledger's ranges
need re-pinning before they are next touched. THG2's range happens to be
unchanged.
...
LANGUAGE.md:1173: "A quoted field name prints in the quotes it is written
with." Repro (`D1.vox`):
...
LANGUAGE.md:1558–1559: "The same error names a function, a parameter, a
...
The sibling owner-return diagnostic (LANGUAGE.md:1310–1313) deliberately
...
LANGUAGE.md:1717–1725 documents the default-must-match-the-field-type
check. Repro (`D4.vox`):
...
### 5. The `--shared` thing-crossing diagnostics locate their caret by searching the source text for the function's name
...
only (LANGUAGE.md:1664–1666), and the text is exactly right. The
...
1. every free call uses the preposition `of`; `to`, `with` and `on` never
   appear in a call (LANGUAGE.md:1377 says all four introduce arguments)
2. every member call passes its arguments with `with`, and the receiver
   call passes none at all (LANGUAGE.md:1396–1397 permits further
...
4. every thing definition indents its entries by exactly four spaces
   (LANGUAGE.md:1679–1681 says indentation is not required)
...
**Two things the master should know that are outside this section.**
First, `INDEX.md` pins manual **0.4.7** and the manual is now **0.4.8**
(5112 → 5238 lines); every ledger's line range needs re-pinning. Second,
running `docs/check-probes.sh` over *all* ledgers gives **123 passed, 4
failed** — and all four failures are in `buffers/` and `values/`, not
here: `BUF-14.vox` and `buffers/D2.vox` no longer print `Text (dynamic)`,
and `values/D1.vox` no longer segfaults (expected exit 139, got 0). Those
look like 0.4.8 fixing two of the open discrepancies, which is good news,
but the rows and the probes need re-running before anyone trusts them.
