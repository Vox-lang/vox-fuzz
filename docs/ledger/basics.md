...
| BAS-46 | 201–209 | The one exception: a blank line placed **after a comma** (mid-sentence) is still just visual spacing. | emit a blank line after a comma inside a multi-action sentence and assert the sentence still completes | yes — the loop's own counter | none — `'gen line piece'` emits `,\n` with no possibility of a blank line after it | todo — hand-verified, `BAS-46.vox` | |
