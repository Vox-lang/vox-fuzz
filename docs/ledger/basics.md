...
| BAS-11 | 76 | A comment can appear in the **middle** of a statement, between tokens. | splice `(...)` between two tokens of a declaration, an expression, and an `If` condition | yes | `gen leaf comment mid declaration` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 12/200 |
| BAS-12 | 77 | A comment can span multiple lines. | emit a `(...)` spanning two or more physical lines between statements | yes — and it is worth doing precisely because the spanned lines are **not** paragraph breaks (BAS-24) | `gen leaf comment multiline nested` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 12/200 |
...
| BAS-19 | ? | A function definition is closed by a **blank line**, and this is *required*, not a style convention. | emit `To … / body / blank line / call`, assert the call ran | yes — `If ticks is not 1 then, Exit 95.` | `'gen emit prelude functions'` (src/gen_core.vox:843–860) and `'gen emit prelude grid sinks'` (src/gen_flow.vox:503) close every definition with `\n\n` | **exercised** — every program relies on it; nothing asserts it | |
| BAS-20 | ? | A period closes only the innermost open clause, so the period ending a body statement does **not** close the function. | folded into BAS-21's shape | yes — same counter | none (the generator only ever writes the closing blank line, so the contrast is never emitted) | todo — hand-verified inside `BAS-21.vox` | |
| BAS-21 | ? | Without a blank line after the body, every following statement is absorbed into the function body; the program then silently does nothing — exit 0, no output. | not a leaf need — this is the shape a generated program must never have | **no** — a generated program that silently does nothing is exactly what a "no output" finding looks like; emitting it deliberately would poison the runner's triage | n/a | not assertable (must-not-generate) — hand-verified, `BAS-21.vox` | |
| BAS-22 | ? | A following `To` or `Library` does begin a new top-level construct and so ends an open function body. | emit two adjacent `To` definitions with no blank line between them and assert both are callable | yes — call both, assert both ran | **none** — every definition the prelude emits is followed by a blank line, so the `To`-ends-`To` path is never taken. `Library` never appears in a generated program at all. | todo — hand-verified for both `To` and `Library`, `BAS-22.vox` | |
| BAS-23 | ? | The compiler warns when a function definition is still open at end of file. | not a leaf need (compile-time diagnostic) | no — a warning is compiler stderr, not program behaviour; the fuzzer's oracle is the compiled program | n/a | not assertable — hand-verified; warning text recorded in `BAS-21.vox`'s header | |
| BAS-24 | 79–83 (undocumented precision) | *(gap)* "Blank line" means whitespace: a line of nothing but **spaces** is a paragraph break and closes a definition; a **comment-only** line is not and does not. | if comments are ever emitted (BAS-07), never emit one where a paragraph break is meant; and vary blank lines between empty and whitespace-only | yes — the same counter as BAS-19 | `gen leaf comment only line does not close` | **verified** | leaf assertion, campaign 2026-09-05 (seed 20260905, 200 programs, budget 12): fired 75/200 |
...
LANGUAGE.md:155 (rule 2): "A blank line (paragraph break) force-closes
...
line (paragraph break) — this is *required*". LANGUAGE.md:243: "An
...
- a nested construct inside a loop body ends with its own period, and the
  ...
  LANGUAGE.md:136–138, BAS-35/BAS-36/BAS-37
...
| indentation is always 2 spaces per nesting level, or 4 in a prelude body | LANGUAGE.md:226 says leading whitespace is ignored entirely — BAS-50 |
...
| no program contains stacked periods (`..`) | LANGUAGE.md:213 documents them — BAS-40/BAS-47 |
| no program contains `Otherwise,.` | LANGUAGE.md:273 documents it — BAS-54 |
...
| no block is ever closed by a blank line | LANGUAGE.md:155, ? — BAS-41/BAS-43 |
